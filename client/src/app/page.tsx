...
import { DashboardProps, Product, ProductCardProps, ProductListProps, SearchableSelectProps } from "@/lib/type";
import { GoogleMap } from "@/lib/map";

const SearchableSelect: React.FC<SearchableSelectProps> = ({ items, value, onChange, placeholder }) => {
...
const ProductList: React.FC<ProductListProps> = ({ products, onSelect, selectedId, stateLabel }) => {
  if (!products.length) {
    return (
...
const CollectorDashboard: React.FC<DashboardProps> = ({ contract, account }) => {
...
const TransporterDashboard: React.FC<DashboardProps> = ({ contract, account }) => {
  const [availableProducts, setAvailableProducts] = useState<Product[]>([]);
  const [selectedProductId, setSelectedProductId] = useState<number | null>(null);
  const [error, setError] = useState("");
  const [loading, setLoading] = useState(false);
                ...
                <div className="flex justify-between items-start">
                  <div className="space-y-2">
                    ...
                    <div className="text-sm text-gray-500">
                      <p>Drop/Shipping Location: {product.localBody}, {product.district}</p>
                      <p>Distance: {product.distance?.toString()} km</p>
                    </div>
                  </div>
                  <div className="text-right space-y-1">
                    <p className="text-sm">Collector Fee: {product.collectorFee} ETH</p>
                    <p className="text-sm">Transport Fee: {product.transporterFee} ETH</p>
                    <p className="font-medium">Total Fee: {product.totalFee} ETH</p>
                  </div>
                </div>
              </div>
            ))}
...
const DistributorDashboard: React.FC<DashboardProps> = ({ contract, account }) => {
  const [availableProducts, setAvailableProducts] = useState<Product[]>([]);
  const [selectedProductId, setSelectedProductId] = useState<number | null>(null);
  const [distributorFee, setDistributorFee] = useState("");
  const [error, setError] = useState("");
...
const TransactionHistory: React.FC<DashboardProps> = ({ contract, account }) => {
        ...
        <div className="space-y-6">
          {transactions.map((tx, index) => (
            <div key={tx.transactionHash} className="relative">
              ...
              <div className="flex gap-4">
                {/* Icon circle */}
                <div className={`flex-shrink-0 w-12 h-12 rounded-full ${getStateColor(tx.state)} flex items-center justify-center`}>
                ...
                <div className="flex-grow bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="text-sm text-gray-600">
                    Status changed to: <span className="font-medium">{getStateName(tx.state)}</span>
                  </div>
                  
                  <div className="mt-2 text-xs text-gray-400 truncate">
                    Tx: {tx.transactionHash.substring(0, 6)}...{tx.transactionHash.substring(tx.transactionHash.length - 4)}
                  </div>
                </div>
              </div>
            </div>
